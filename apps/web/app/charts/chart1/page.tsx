...
import { CoinSearch } from './componet/coinSearch';
const Chart1Page = () => {
  const [selectedCoin, setSelectedCoin] = useState<string>('KRW-BTC');
  const [selectedTimeRange, setSelectedTimeRange] = useState<string>('24h');
  const [coins, setCoins] = useState<string[]>(['KRW-BTC', 'KRW-DOGE', 'KRW-XRP']);
          ...
          {coins.map((coin) => (
            <li key={coin} style={{ marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ background: 'none', border: '1px solid #ccc', borderRadius: '4px', padding: '2px 6px', cursor: 'pointer' }}
              >
                -
              </button>
            </li>
